...
class IotService:
    ...
    def create_user(username, email, password, full_name , bio):
        ...
        new_user = user(user_id=username, email=email, password_hash=hashed_password, name=full_name)
    ...
    def add_location(location_id, state, city, zip):
        new_location = Location(location_id=location_id, state=state, city=city, zip_code=zip)
    ...
    @staticmethod
    def get_sensorData_by_time_interval(start_time, end_time):
        return SensorData.query.filter(SensorData.sensor_time >= start_time, SensorData.sensor_time <= end_time).all()